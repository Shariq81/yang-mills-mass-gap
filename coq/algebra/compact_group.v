...
Class HaarIntegral (G : Type) `{Group G} := {
  ...
  (* Linearity *)
  haar_linear_add   : forall f g, haar (fun x => (f x + g x)%R) = (haar f + haar g)%R;
  haar_linear_scale : forall c f, haar (fun x => (c * f x)%R) = (c * haar f)%R;

  (* Positivity *)
  haar_nonneg : forall f, (forall x, f x >= 0) -> haar f >= 0;
  
  (* Normalization *)
  haar_normalized : haar (fun _ => 1%R) = 1%R;
...
Class HeatKernelPosDef (G : Type) `{Group G} `{WilsonClassFunction G} := {
  heat_kernel : R -> G -> G -> R := 
    fun beta g h => exp (beta * phi (gmul g (ginv h)))%R;
