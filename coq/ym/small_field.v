...
(* 4b. Max activity term for a=1: |activity q| * exp(size q) = exp(-β/10) * exp(1) *)
Definition ym_max_term (p : YMPolymer) : R :=
  Rabs (ym_activity p) * exp (1 * 1).

Lemma ym_max_term_eq : forall p,
  ym_max_term p = exp (1 - beta / 10).
Proof.
...
Lemma ym_neighbor_term_bound : forall p q,
  In q (neighbors p) ->
  Rabs (activity q) * exp (1 * size q) <= ym_max_term p.
Proof.
  intros p q Hin.
  (* All polymers have the same activity and size in our YM model *)
  (* activity q = ym_activity q = exp(-beta/10), size q = 1 *)
  unfold ym_max_term.
  simpl activity. simpl size.
  (* Both sides are now Rabs(ym_activity _) * exp(1*1), so equal *)
  right. reflexivity.
Qed.
...
(* Explicit KP bound: kp_sum_explicit 1 p <= 24 * ym_max_term p *)
Lemma ym_kp_sum_explicit_bounded : forall p,
  kp_sum_explicit 1 p <= 24 * ym_max_term p.
Proof.
  intros p.
...
Lemma kp_arithmetic_at_50 : 24 * exp(1 - 50/10) <= 1.
Proof.
  replace (1 - 50/10) with (-(4)) by lra.
  rewrite exp_Ropp.
...
(* THE KEY LEMMA: explicit KP sum <= 1 for beta > 50 *)
Lemma ym_kp_sum_explicit_le_1 : beta > 50 -> forall p, kp_sum_explicit 1 p <= 1.
Proof.
  intros Hbeta p.
  apply Rle_trans with (24 * ym_max_term p).
  - apply ym_kp_sum_explicit_bounded.
  - rewrite ym_max_term_eq.
    apply Rle_trans with (24 * exp(1 - 50/10)).
    + apply Rmult_le_compat_l; [lra |].
      apply exp_monotone_neg; lra.
    + apply kp_arithmetic_at_50.
Qed.
...
Theorem ym_satisfies_kp :
  beta > 50 ->
  @kp_sum_condition YMPolymer YM_PolymerSystem YM_MetricSystem YM_SummationSystem 1.
Proof.
  intros Hbeta. unfold kp_sum_condition, kotecky_preiss_condition.
  intros p. rewrite <- kp_sum_explicit_eq_sum_overlap.
  apply Rle_trans with 1.
  - apply ym_kp_sum_explicit_le_1. exact Hbeta.
...
Theorem ym_polymer_mass_gap_from_kp :
  beta > 50 -> has_mass_gap (correlator ym_polymers_Λ ym_N_max ym_connects_dec) 1.
Proof.
  intros Hbeta.
  (* exponential_decay_from_cluster takes:
     - polymers_Λ, N_max, connects_dec (Context vars)
     - connects_implies_dist_bound (Section Hypothesis)
     - a, S_max, and 4 proof obligations
     Note: connecting_bounded_by_pinned is an Admitted Theorem in the Section,
           so it's NOT a parameter - it's used internally. *)
  ...
  - lra.
  - intros p. simpl. lra.
  - apply ym_satisfies_kp. exact Hbeta.
Qed.
